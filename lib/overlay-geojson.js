...
function drawLines (ctx, lines, imgLeftTop, zoom, scale) {
  ...
  imgLines.forEach(line => {
    ...
    line.geometry.coordinates.forEach(([x,y], i) => {
      ...
      ctx.lineJoin = 'round';
      // full map
      //ctx.strokeStyle = line.properties.drawOrder % 2 === 0 ? '#943D3D' : '#B56045';
      //ctx.strokeStyle = '#049400';
      ctx.strokeStyle = '#A11F1F';
      // key map
...
async function drawPoints (ctx, points, imgLeftTop, zoom, scale) {
  ...
  for (const point of imgPoints) {
    ...
    if (point.properties.Size === 'big') {
      markerImg = await loadImage(process.cwd() + "/icons/marker-red-location.png");
      width = markerImg.width * 1.5;
      height = markerImg.height * 1.5;
    } else {
      markerImg = await loadImage(process.cwd() + "/icons/marker-yellow-location.png");
...
async function drawLabels (ctx, points, imgLeftTop, zoom, scale) {
  ...
  for (const point of imgPoints) {
      const [x, y] = point.geometry.coordinates;
      console.log('drawing lables at: ', x, y);
      xpos = x + 10;
      if (point.properties.Name === 'Ghaziabad, UP') { 
        ypos = y -40;
      } else {
        ypos = y + 30;
      }
...
/*
(async () => {
  // const geojson = jsonfile.readFileSync(process.cwd() +'/icons/sample.geojson');
